...
public class ConfigUtil {
    private static final Set<Class<?>> brokenClasses = ConcurrentHashMap.newKeySet();
    ...
    public static void saveConfig(Object config, File file, Gson gson, boolean useGzip) {
        ...
        synchronized (getLock(file)) {
            ...
            try {
                ...
                if (!tempFile.exists()) {
                    WynnAdhocClient.LOGGER.error("Temp file disappeared before move: " + tempFile);
                    return;
                }
                ...
                try {
                    ...
                    WynnAdhocClient.LOGGER.warn("Atomic move failed attempting normal" + e.getMessage());
    ...
    private static void makeBackup(File file, String suffix) {
        if (!file.exists()) {
            WynnAdhocClient.LOGGER.warn("Cannot make backup of non-existent file: " + file.getName());
            return;
        }
        ...
        try {
            ...
            try {
                ...
                WynnAdhocClient.LOGGER.error("Normal move failed " + e2.getMessage());
            }
        }
    }
}